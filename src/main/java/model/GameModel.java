...
import strategy.BoomerangMoveStrategy;
...
public class GameModel implements IObservable, IGameModel {
    ...
    public GameModel() {
        this.activeMovementStrategy = new ParabolicMoveStrategy();
    ...
    public Object createMemento() {
        ...
        memento.cannon = this.cannon.clone();

        for (Enemy e : this.enemies) {
            memento.enemies.add(e.clone());
        }
        for (Missile m : this.missiles) {
            memento.missiles.add(m.clone());
        }
        for (Collision c : this.collisions) {
            memento.collisions.add(c.clone());
        }
        return memento;
    }
    ...
    public void boomerangShootingStrategy() {
        ...
        this.activeMovementStrategy = new BoomerangMoveStrategy();
