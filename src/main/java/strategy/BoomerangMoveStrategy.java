 ...
 * @author Jakub Trhlík
 */
public class BoomerangMoveStrategy implements IMoveStrategy {

    @Override
    public int nextPositionY(int initY, float initAngle, float initForce, long lifetime) {
        return (int)(initY +(initForce * lifetime * Math.sin(initAngle)) + (lifetime/50)*(lifetime/50));
    }

    @Override
    public int nextPositionX(int initX, float initAngle, float initForce, long lifetime) {
        return (int)(initX +(initForce * lifetime * Math.cos(initAngle)) - initForce*lifetime*lifetime/1000);
    }
